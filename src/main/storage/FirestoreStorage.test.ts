...
describeFirestore('FirestoreStorage lifecycle', () => {
...
describeFirestore('FirestoreStorage contract', () => {
  runIstorageContractSuite('FirestoreStorage', createTestDb);
});

describeFirestore('FirestoreStorage reorder ownership', () => {
...
describeFirestore('FirestoreStorage import and ID allocation', () => {
  /**
  ...
  it('nextId dispenses IDs from cached blocks instead of one transaction per insert', async () => {
    ...
    const allocateIdsSpy = vi.spyOn(
      FirestoreStorage.prototype as unknown as {
